...
const name$1 = "strapi-plugin-strapi-algolia";
const version = "2.0.4";
const description = "Index articles configured to Algolia";
const keywords = [
  "strapi",
  "plugin",
  "algolia"
];
const homepage = "https://market.strapi.io/plugins/strapi-plugin-strapi-algolia";
const repository = {
  type: "git",
...
const init = (initialState2, shouldFetchData) => ({
  ...initialState2,
...
var Actions = /* @__PURE__ */ ((Actions2) => {
  Actions2["GET_DATA"] = "GET_DATA";
  Actions2["GET_DATA_SUCCEEDED"] = "GET_DATA_SUCCEEDED";
  Actions2["GET_DATA_ERROR"] = "GET_DATA_ERROR";
  return Actions2;
})(Actions || {});
const initialState = {
  contentTypes: [],
...
const reducer = (state, action) => fn(state, (draftState) => {
  switch (action.type) {
    case "GET_DATA": {
...
const useConfigContentTypes = (shouldFetchData = true) => {
  const [{ contentTypes, isLoading }, dispatch] = react.useReducer(
    reducer,
    initialState,
  ...
  const fetchConfigContentTypes = async () => {
    try {
      ...
      const message = err?.response?.payload?.message ?? "An error occurred";
...
const IndexAllButton = ({ contentType }) => {
      ...
      designSystem.Dialog.Root,
      {
        ...
        children: /* @__PURE__ */ jsxRuntime.jsx(
          admin.ConfirmDialog,
          {
            ...
            variant: "default",
            icon: /* @__PURE__ */ jsxRuntime.jsx(icons.WarningCircle, { fill: "danger500" }),
            children: formatMessage({
              id: "cache.purge.confirm-modal-body",
              defaultMessage: "Are you sure you want to index all items this content type?"
            })
          }
        )
      }
...
const ListViewInjectedComponent = () => {
  const routeMatch = reactRouterDom.useMatch("/content-manager/:kind/:slug?");
  const { allowedActions } = admin.useRBAC(pluginPermissions);
  if (!allowedActions.canIndexAll) {
    return null;
  }
  return /* @__PURE__ */ jsxRuntime.jsx(IndexAllButton, { contentType: routeMatch?.params.slug });
};
...
const prefixPluginTranslations = (trad, pluginId2) => {
  ...
  return Object.keys(trad).reduce(
    (acc, current) => ({
      ...acc,
      [`${pluginId2}.${current}`]: trad[current]
    }),
    {}
  );
};
const index = {
  ...
  async registerTrads({ locales }) {
    const importedTranslations = await Promise.all(
      locales.map(
        (locale) => __variableDynamicImportRuntimeHelper(/* @__PURE__ */ Object.assign({ "./translations/en.json": () => Promise.resolve().then(() => require("../_chunks/en-Bk6zo4Dt.js")), "./translations/fr.json": () => Promise.resolve().then(() => require("../_chunks/fr-Du6Nbrj6.js")) }), `./translations/${locale}.json`, 3).then(({ default: data }) => ({
